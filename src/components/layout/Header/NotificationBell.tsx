          ...
          {status === 'success' && notifications.length > 0 && (
            <div className="border-background-border mt-3 flex items-center justify-start border-t px-5 pt-3">
              <button
                ...
                disabled={!hasUnread}
                className={cn(
                  'text-brand-500 hover:text-brand-600 rounded-sm text-xs font-semibold transition-colors hover:underline disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:no-underline',
                  FOCUS_RING_CLASSES
                )}
              >
                Mark all as read
