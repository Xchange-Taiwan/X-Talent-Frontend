...
import { categorizeChecks, isTimeout, parseAIReviewComments, filterIgnoredChecks } from "./pr-parser.mjs";
...
if (!prTarget) {
  ...
  try {
    ...
    console.log(cyan(`PR URL: ${prTarget}\n`));
  } catch (err) {
...
while (true) {
  ...
  try {
    // 1. Fetch and Parse AI Review Comments on the PR safely
        ...
        if (finding.issues.length > 0) {
          ...
          for (const issue of finding.issues) {
            ...
            if (issue.description) {
              console.log(yellow(`        ${issue.description}`));
            }
          }
        } else {
    ...
    // 2. Fetch and Parse Pipeline Checks safely
    ...
    if (checks.length === 0) {
      ...
      const { critical: failing, ignored: ignoredFails } = filterIgnoredChecks(rawFailing);
